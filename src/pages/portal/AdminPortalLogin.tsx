...
export default function AdminPortalLogin() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const { data, error } = await supabase.functions.invoke("resolve-admin-id", {
        body: { admin_id: adminId.trim() },
      });
    ...
    <div className="min-h-screen relative flex items-center justify-center px-4 overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-xs tracking-[0.25em] text-accent">— ADMIN PORTAL —</p>
        </div>
        <h1 className="font-display text-3xl mb-1">Administrator sign-in</h1>
        <p className="text-sm text-muted-foreground mb-6">Restricted access — administrators only.</p>

        <form onSubmit={submit} className="space-y-3">
          <label className="block">
            <span className="text-[11px] uppercase tracking-widest text-muted-foreground flex items-center gap-1"><IdCard className="w-3 h-3" /> Admin ID</span>
            <input
              value={adminId} onChange={(e) => setAdminId(e.target.value.toUpperCase())}
              required placeholder="ADM-0001"
        ...
        <p className="mt-5 text-xs text-center text-muted-foreground">
          Wrong portal? <Link to="/auth" className="text-accent hover:underline">Patient</Link>
          {" · "}
          <Link to="/portal/doctor" className="text-accent hover:underline">Doctor</Link>
        </p>
      </motion.div>
    </div>
  );
